...
On obtient donc la suite $x_{n+1}=x_n-hLx_n=x_n(1-hL)$ donc on peut montrer par récurrence que $x_{n}=x_0 (1-hL)^{n}$.\\
Le schéma est A-stable si $x_n$ tend vers 0 quand n tend vers l'infini, c'est à dire si et seulement si:

\begin{displaymath}\begin{array}{l}-1<1-hL<1\\-2<-hL<0\\0<hL<2\\h<2/L\\\end{array}\end{displaymath}
...
On voit que dans les 3 cas, h est soumis à conditions par rapport à L: Les schémas ne sont donc pas inconditionnellement stables. C'est souvent le cas, pour des schémas explicites.
