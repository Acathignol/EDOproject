...
Lors de calculs de grands nombres, on aboutit systématiquement non pas à une erreur indiquant que l'on dépasse une éventuelle capacité de calcul maximale, mais à une \textit{erreur de segmentation}. Matlab préfère donc le crash sans sauvegarde possible à toute autre alternative. \\Et cet incident n'est pas ponctuel: lorsqu'un algorithme ne passe pas, il ne passe pas, même après modification et remodification, nous forçant donc à une dizaine de kills du processus avant de trouver un programme stable.\\

On pourrait penser que passé un certain tarif de licence, les développeurs auraient le tact de vérifier leur code. 
\begin{table}[h]
\begin{tabular}{cc}
...
\textbf{Conclusion: Nous sommes venus utiliser MatLab sans préjugés, nous repartons avec. Python, avec toute sa lenteur, fonctionne sur tous les ordinateurs gratuitement et sans problèmes GRAVES.}
