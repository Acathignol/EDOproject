...
On sait que : \begin{equation}
\phi(t_n,x_n,h_n)=\Sigma^{s}_{i=1} b_i f(t_n+c_i h_n,x_i)
\end{equation}
Et \begin{equation}
\frac{\delta}{\delta h}\phi(t,x,0)=\frac{1}{2}Df(t,x)
\end{equation}
Et, $x_i=x_n+h_n\Sigma^{i-1}_{j=1} a_{i,j} f(t_n+c_jh_n,x_j)$ .\\
Alors:\begin{equation}
\frac{\delta x_i}{\delta h}=\frac{\delta}{\delta h}(x_n+h_n\Sigma^{i-1}_{j=1} a_{i,j} f(t_n+c_jh_n,x_j))
\end{equation}
\begin{equation}
\Leftrightarrow \frac{\delta x_i}{\delta h}=\Sigma^{i-1}_{j=1} a_{i,j} f(t_n+c_jh_n,x_j))
\end{equation}

De plus:
\begin{equation}
\frac{\delta}{\delta h}[b_if(t_n+c_ih_n,x_i)] = b_i \frac{\delta}{\delta h}f(t_n+c_ih_n,x_i)
\end{equation}
\begin{equation}
\Leftrightarrow \frac{\delta}{\delta h}[b_if(t_n+c_ih_n,x_i)]= b_i (\frac{\delta f}{\delta (t_n+c_ih_n)}(PUT)\frac{\delta (t_n+c_ih_n)}{\delta h}+\frac{\delta f}{\delta x_i}(PUT) \frac{\delta x_i}{\delta h})
\end{equation}
\begin{equation}
\Leftrightarrow \frac{\delta}{\delta h}[b_if(t_n+c_ih_n,x_i)]= b_i (\frac{\delta f}{\delta (t_n+c_ih_n)}(PUT)c_i+\frac{\delta f}{\delta x}(PUT) \frac{\delta x_i}{\delta h})
\end{equation}


Donc, comme $\phi(t_n,x_n,h_n)=\Sigma^{s}_{i=1} b_i f(t_n+c_i h_n,x_i)$,
on obtient \\
\\
$ \frac{\delta}{\delta h}\phi(t_n,x_n,h_n) = \frac{\delta}{\delta h}[\Sigma^{s}_{i=1} b_i f(t_n+c_i h_n,x_i)]\\
\\
\Leftrightarrow \frac{\delta}{\delta h}\phi(t_n,x_n,h_n) = (\Sigma^{s}_{i=1} b_ic_i) \frac{\delta f}{\delta (t_n+c_ih_n)}(PUT) + [\Sigma^{s}_{i=1} b_i(\Sigma^{i-1}_{j=1} a_{i,j} f(t_n+c_jh_n,x_j))]  \frac{\delta f}{\delta x}\\ $
\\
Donc, si $h_n=0$ :
\begin{equation}
\frac{\delta}{\delta h}\phi(t_n,x_n,0) = (\Sigma^{s}_{i=1} b_ic_i) \frac{\delta f}{\delta t}(t,x) + [\Sigma^{s}_{i=1} b_i(\Sigma^{i-1}_{j=1} a_{i,j})] f(t,x) \frac{\delta f}{\delta x}(t,x)
\end{equation}
Soit $Df(t,x)=\frac{\delta f}{\delta t}(t,x)+f(t,x) \frac{\delta f}{\delta x}(t,x)$.\\
Et, on sait que $\frac{\delta}{\delta h}\phi(t,x,0)=\frac{1}{2}Df(t,x)$.\\
\begin{equation}
\Leftrightarrow \frac{\delta}{\delta h}\phi(t,x,0) = \frac{1}{2}\frac{\delta f}{\delta t}(t,x)+\frac{1}{2}f(t,x) \frac{\delta f}{\delta x}(t,x)
\end{equation}
$\Leftrightarrow$\[
\left\{
\begin{array}{ll}
\Sigma^{s}_{i=1} b_ic_i = \frac{1}{2} \\
\Sigma^{s}_{i=1} b_i(\Sigma^{i-1}_{j=1} a_{i,j}) = \frac{1}{2}\\
\end{array}
\right.
\]

Donc, pour un modèle de Runge-Kutta explicite à un pas soit au-moins d’ordre 2, il faut et
il suffit que les points $b_i$ , les poids intermédiaires $a_{i,j}$ et les paramètres $c_i$ $(i, j = 1, ..., s)$
vérifient toutes les conditions suivantes: 
\[
\left\{
\begin{array}{ll}
\Sigma^{s}_{i=1} b_ic_i = \frac{1}{2} \\
...
\end{tabular}\\
\begin{tabular}{cccc}
\subfloat[T=15, $y_0$=1]{\includegraphics[scale=0.4]{ex3_3_h1T15x01.png}}
&
\subfloat[T=20, $y_0$=1]{\includegraphics[scale=0.4]{ex3_3_h1T20x01.png}}
&
\subfloat[T=20, $y_0$=1000]{\includegraphics[scale=0.4]{ex3_3_h1T20x01000.png}}
&
\subfloat[T=50, $y_0$=1]{\includegraphics[scale=0.4]{ex3_3_h1T50x01.png}}
\\
\end{tabular}
\begin{tabular}{l}
Pour $h=0.3$:
\end{tabular}\\
\begin{tabular}{cccc}
\subfloat[T=15, $y_0$=1]{\includegraphics[scale=0.4]{ex3_3_h3T15x01.png}}
&
\subfloat[T=20, $y_0$=1]{\includegraphics[scale=0.4]{ex3_3_h3T20x01.png}}
&
\subfloat[T=20, $y_0$=1000]{\includegraphics[scale=0.4]{ex3_3_h3T20x01000.png}}
&
...
\end{tabular}\\
\begin{tabular}{ccc}
\subfloat[T=15, $y_0$=1]{\includegraphics[scale=0.4]{ex3_3_h5T15x01.png}}
&
\subfloat[T=15, $y_0$=2]{\includegraphics[scale=0.4]{ex3_3_h5T15x02.png}}
&
\subfloat[T=20, $y_0$=2]{\includegraphics[scale=0.4]{ex3_3_h5T20x02.png}}
\\
\end{tabular}
\end{table}
\begin{table}
...
\\
\begin{tabular}{cc}
\subfloat[Chronique de $y$ avec \texttt{ode23s()}.]{\includegraphics[scale=0.5]{exo6_3_1.png}}
&
\subfloat[Chronique de $y$ avec \texttt{ode45()}.]{\includegraphics[scale=0.5]{exo6_3_ode45_9.png}}
\\
\end{tabular}
\end{table}
